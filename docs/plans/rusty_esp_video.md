...
| **V1** (J1) ◐ host half 2026-09-01 | MJPEG over HTTP from XIAO S3 Sense, Track A. **Done on the host:** `net::MjpegHttpServer` over `std::net` (the code the board runs), `EncodedSource`, `mjpeg_reader::Reader` + `client::pull_stream` + `mjpeg_record` (the Pi record path, ffprobe-verified), the firmware project `firmware/xiao-s3-sense-idf-mjpeg` (written; ESP-IDF build in progress), ffmpeg reads 8 MJPEG frames from the host server (`docs/LEDGER.md`) | `http://device/stream` opens in a browser at 320×240 at a recorded FPS; recorded to disk on a Pi (pi-mission H3) — **needs the board** |
