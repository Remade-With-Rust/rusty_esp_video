...
|---|---|---|
| H.264 encode on-chip | `rusty_h264` branch `no-std` (PR #7) with `default-features = false, features = ["libm"]`: `no_std` + `alloc`, scalar, `forbid(unsafe)`, no build script, no `rusty_alloc` hijack | the `no_std` pass is done (2026-09-02); still owed upstream: a borrowed-frame input path (the encoder takes an owned `YuvFrame` and returns an owned `Vec`) and the decoder's own `no_std` |
...
| 2026-09-02 | `h264` implied `std` for four hours; the upstream `no_std` pass landed on branch `no-std` the same day and `h264` is now `alloc` + `rusty_h264/libm`. The wrapper did not change — it was written against the public API only. Janus depends on the branch by git (umbrella patch to `../rusty_h264`) until it is released. |
