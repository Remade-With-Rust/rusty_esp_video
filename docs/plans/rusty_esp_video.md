...
|---|---|---|
| H.264 encode on-chip | `rusty_h264` **0.14 from crates.io** with `default-features = false, features = ["libm"]`: `no_std` + `alloc`, scalar, `forbid(unsafe)`, no build script, no `rusty_alloc` hijack | done upstream and adopted (2026-09-03): `EncoderConfig::baseline`, borrowed `YuvPlanes` in, `encode_into` writes the access unit in place, `request_keyframe`; still upstream: the decoder's own `no_std` |
...
| **V2** ◐ host half 2026-09-02 (software complete) | RTP/JPEG (RFC 2435) and raw-UDP framing to a laptop; `Pacer`. **Done on the host:** `rtp::JpegDepayloader` + the Annex K table set, `udp_net::{RtpJpegSender, RawUdpSender, receive_rtp_jpeg, receive_raw}`, examples `rtp_send` / `rtp_recv`; **the drop policy**: `pacer::Budget` (a byte token bucket; a frame that does not fit is dropped whole and counted, never sent late or in part) on both senders (`with_budget(kbps, burst_ms)`), `--kbps` on `rtp_send`; ffmpeg reads our RTP/JPEG and we read ffmpeg's; ten minutes sender→receiver on the Wi-Fi address: 5 989 frames, 0 lost, 0 dropped | a laptop receiver reassembles 10 minutes with the loss counter recorded — **done host-to-host** (ledger); the board as sender is the row in `hardware-verify.md` |
| **V3** (J5) ◐ host half 2026-09-02 | `rusty_h264` `no_std` + Baseline I/P at QVGA; H.264 in TS over UDP. **Done on the host:** `encoder::H264` behind `VideoEncoder` (feature `h264`, Constrained Baseline CAVLC, no lookahead, fixed GOP), the QVGA oracle (`ffprobe` reads `h264,Constrained Baseline,320,240,30`, house decoder round-trip, IDR on the GOP, 439/475 us per frame on the host), the codec **policy** (`policy::{Job, codec_for}`); the upstream `no_std` pass **done** on branch `no-std` ([PR #7](https://github.com/Remade-With-Rust/rusty_h264/pull/7)) and `h264` here is `alloc` + `libm`: **checks on `riscv32imac` and `riscv32imafc`** | `rff -i udp://@:1234` plays it; FPS, bitrate and cycle budget written in the ledger honestly (S3 software) - **needs the board** |
...
| 2026-09-02 | `h264` implied `std` for four hours; the upstream `no_std` pass landed on branch `no-std` the same day and `h264` is now `alloc` + `rusty_h264/libm`. The wrapper did not change — it was written against the public API only. Janus depended on the branch by git until it was released. |
| 2026-09-03 | `rusty_h264` 0.14 and `rusty_jpeg` 0.4 from crates.io, no git pins left. The wrapper moved to the chip API the plan asked for: `EncoderConfig::baseline` (the same constructor `rff -profile baseline -preset fast` selects, so host and device bytes agree), the seam's `Frame` borrowed as `YuvPlanes` with its strides, `encode_into` straight into the packetizer's buffer, `request_keyframe` instead of a fresh encoder. `jpeg::SoftJpeg` joins the seam for sensors without a JPEG mode. rff's `rtp://` and `mpjpeg` inputs are the second external oracle beside ffmpeg (ledger). |
| 2026-09-02 | `libm` on both sides: with it the host's float decisions match the chip's bit for bit, and on this host the QVGA oracle's bytes did not change at all against the platform libm. |
| 2026-09-02 | **RFC 2435 means Annex K Huffman tables, so the sender checks.** The wire carries no DHT and every receiver regenerates the standard set; `JpegScan::parse` refuses an optimised-table JPEG rather than ship a stream nobody can decode. `rusty_jpeg` writes the standard set by default and the OV2640 always does. |
| 2026-09-02 | **Drop the frame that does not fit, never send it late.** `Budget` is a token bucket in wire bytes; a live stream gains nothing from a frame that arrives after the next one, and a queue only turns congestion into latency. Sizing is the caller's: at 10 fps and 6 kB a stream is 480 kbit/s, a 200 kbit/s cap admits about four frames in ten, and the counters say which. |
| 2026-09-02 | The receiver rebuilds a JPEG **in place**: the scan lands in the caller buffer after a header reserve and the headers are written in front of it on the marker packet, so a complete frame is never copied. Loss is reported, never guessed. |
