...
| **V2** ◐ host half 2026-09-02 | RTP/JPEG (RFC 2435) and raw-UDP framing to a laptop; `Pacer`. **Done on the host:** `rtp::JpegDepayloader` + the Annex K table set, `udp_net::{RtpJpegSender, RawUdpSender, receive_rtp_jpeg, receive_raw}`, examples `rtp_send` / `rtp_recv`; ffmpeg reads our RTP/JPEG and we read ffmpeg's; ten minutes sender→receiver on the Wi-Fi address: 5 989 frames, 0 lost, 0 dropped | a laptop receiver reassembles 10 minutes with the loss counter recorded — **done host-to-host** (ledger); the board as sender is the row in `hardware-verify.md` |
